...
function sendFriendRequest(userId, friendId) {
	return db.prepare(`INSERT INTO friends (userId1, userId2, status) VALUES (?, ?, 'pending')`).run(userId, friendId);
}

function acceptFriendRequest(userId, friendId) {
	return db.prepare(`UPDATE friends SET status = 'accepted' WHERE (userId1 = ? AND userId2 = ?) OR (userId1 = ? AND userId2 = ?)`).run(userId, friendId, friendId, userId);
}

function blockUser(userId, friendId, blockerId) {
	return db.prepare(`UPDATE friends SET status = 'blocked', blocked_by = ? WHERE (userId1 = ? AND userId2 = ?) OR (userId1 = ? AND userId2 = ?)`).run(blockerId, userId, friendId, friendId, userId);
}

function unblockUser(userId, friendId, blockerId) {
	return db.prepare(`UPDATE friends SET status = 'accepted', blocked_by = NULL WHERE (userId1 = ? AND userId2 = ?) OR (userId1 = ? AND userId2 = ?) AND blocked_by = ?`).run(userId, friendId, friendId, userId, blockerId);
}
...
function getPendingRequests(userId) {
	return db.prepare(`SELECT f.userId1, u.name, u.email FROM friends f
    	JOIN users u ON u.id = f.userId1
    	WHERE f.userId2 = ? AND f.status = 'pending'`).all(userId);
}

function checkFriendshipExists(user1, user2) {
	return db.prepare(`SELECT * FROM friends WHERE (userId1 = ? AND userId2 = ?) OR (userId1 = ? AND userId2 = ?)`).get(user1, user2, user2, user1);
}

function undoFriendship(userId1, userId2) {
	return db.prepare(`DELETE FROM friends WHERE (userId1 = ? AND userId2 = ?) OR 
